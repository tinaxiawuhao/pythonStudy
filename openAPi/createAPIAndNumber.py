...
def api_reader():
    ...
    for i in range(len(list_value)):
        # 插入数据
        if (len(list_value[i][0].strip()) != 0 and len(list_value[i][1].strip()) != 0):
            opration_mysql(list_value[i][0].strip(), list_value[i][1].strip())
            number = number + 1
