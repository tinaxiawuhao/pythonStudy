...
while running:
    ...
    screen.blit(castle_img, (0, 40))
    screen.blit(castle_img, (0, 145))
    screen.blit(castle_img, (0, 250))
    screen.blit(castle_img, (0, 355))
    # 首先获取鼠标和玩家的位置。然后，获取通过atan2函数得出的角度和弧度。
    # 当兔子被旋转的时候，它的位置将会改变。
    # 所以你需要计算兔子新的位置，然后将其在屏幕上显示出来。
    position = pygame.mouse.get_pos()
    angle = math.atan2(position[1] - (playerpos[1] + rabbit_img.get_height() / 2),
                       position[0] - (playerpos[0] + rabbit_img.get_width() / 2))
    playerrot = pygame.transform.rotate(rabbit_img, 360 - angle * 180 / math.pi)
    ...
    for bullet in arrows:
        ...
        for projectile in arrows:
            arrow = pygame.transform.rotate(arrow_img, 360 - projectile[0] * 180 / math.pi)
    ...
    if badtimer == 0:
        ...
        badtimer = 100 - random.randint(10, 30)
    index_badguy = 0
    # 外层循环獾，每次循环前进4格像素
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            position = pygame.mouse.get_pos()
            acc[1] += 1
            arrows.append([math.atan2(position[1] - (playerpos1[1] + rabbit_img.get_height() / 2),
                                      position[0] - (playerpos1[0] + rabbit_img.get_width() / 2)),
                           playerpos1[0] + rabbit_img.get_height() / 2, playerpos1[1] + rabbit_img.get_width() / 2])
    # 移动玩家
    if keys[0]:
        playerpos[1] -= 5
    elif keys[2]:
